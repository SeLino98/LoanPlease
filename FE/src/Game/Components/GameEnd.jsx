...
function GameEnd () {
  ...
  const { score, credit, resetGame } = useStore();
  ...
  return (
    <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-gray-800 bg-opacity-75">
      <div className="p-5 h-[400px] w-[600px] bg-cusColor1 rounded shadow-lg text-white border-[15px] border-cusColor3">
        <p className="text-[40px]">퇴근하세요!</p>
        <br />
        <p className="mt-[30px] text-4xl">SCORE : {score}</p>
        <p className="mt-[30px] text-4xl">CREDIT : {credit}</p>
